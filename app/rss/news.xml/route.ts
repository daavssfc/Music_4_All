...
export async function GET() {
  ...
  try {
    items = await getSanityClient().fetch<NewsItem[]>(
      `*[_type == "post" && defined(slug.current)] | order(publishedAt desc, _updatedAt desc)[0...50]{
        title,
        "slug": slug.current,
        excerpt,
        publishedAt,
  ...
  <rss version="2.0">
    <channel>
      <title>Music 4 All News</title>
      <link>${escapeXml(`${baseUrl}/news`)}</link>
      <description>Latest news from Music 4 All.</description>

...
export async function GET(request: Request) {
  ...
  const query = `{
    "items": *[
      ${typeFilter}
      && (
        (_type == "review" && status == "published" && (title match $query || excerpt match $query || pt::text(body) match $query))
        || (_type == "post" && status == "published" && (title match $query || excerpt match $query || pt::text(body) match $query))
        || (_type == "event" && status == "published" && (title match $query || venue match $query || pt::text(description) match $query))
        || (_type == "artist" && (name match $query || pt::text(bio) match $query))
      )
    ]

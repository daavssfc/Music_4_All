...
import { tagAggregationSchema } from "@/lib/contracts/models";
import { getSanityClient } from "@/lib/sanity/client";

export const runtime = "nodejs";

const tagAggQuerySchema = z.object({
...
export async function GET(request: Request) {
  ...
  const filters = tagAggQuerySchema.parse(Object.fromEntries(searchParams.entries()));
  ...
  const parsed = paginatedResponseSchema(tagAggregationSchema).parse(response);
